...
Auth flow (one-time, ~30 seconds):
    ...
    → token cached to %LOCALAPPDATA%\\Clacky\\github_token.json
...
def _log_login(line: str) -> None:
    ...
    after the fact (the user can grab this from %LOCALAPPDATA%\\Clacky\\)."""
    try:
...
async def refresh_models_to_cache() -> list[dict]:
    """Fetch live + write to %LOCALAPPDATA%\\Clacky\\copilot_models.json."""
...
def pick_default_free_model() -> str:
    """Best free model for Clacky — vision-capable, multiplier 0."""
    models = cached_models()
    # Vision-capable AND free → ideal (Clacky sends screenshots)
    for m in models:
