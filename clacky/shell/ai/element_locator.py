...
# Shared client — keeps the TLS connection warm across locates instead of paying
# a ~0.2-0.4s handshake on every call. Reset on error (e.g. stale after sleep).
...
async def detect_element(
    *,
    ...
    try:
        r = await _get_client().post(_API_URL, json=body, headers=headers)
        if r.status_code >= 400:
            return None
        data = r.json()
    except Exception:
        global _client
        _client = None    # stale pool → rebuild on the next locate
        return None
