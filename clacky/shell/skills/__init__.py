...
level:
    ...
    SKILL = {
        ...
        "description": "Lets Clacky click for you instead of pointing.",
...
def _user_skills_dir() -> Path:
    """User-level skills dir at ~/.clacky/skills/ — survives reinstall."""
    return Path.home() / ".clacky" / "skills"


def load_all() -> list[dict]:
    """Discover + import every skill module in this package and ~/.clacky/skills."""
    global _loaded
    _loaded = []

    # Bundled skills (shipped with Clacky)
...
def _try_import(path: Path) -> None:
    try:
        spec = importlib.util.spec_from_file_location(
            f"clacky_skill_{path.stem}", str(path)
        )
