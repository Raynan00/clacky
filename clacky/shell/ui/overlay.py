"""
Clacky's blue buddy cursor — faithful port of the original macOS overlay.
...
FLY_DURATION_MIN = 0.35   # snappy flight; was 1.6 ("teacher pace")
FLY_DURATION_MAX = 0.7    # was 2.8
...
class CursorOverlay(QWidget):
    ...
    def __init__(self):
        ...
        # Slow mode — doubles flight + dwell so Clacky feels more like a teacher
    ...
    def point_at(self, x: float, y: float, label: str = ""):
        ...
        # Empty label → NO bubble (just the cursor pointing). We used to fill it
        # with a random cheery phrase ("this one!"), but during an explanation that
        # reads as random noise out of sync with the voice. Real labels still show.
        self._bubble_text = label or ""
        self._bubble_scale = 0.5
        self._bubble_alpha = 0.0
        # Halo ring removed — it looked busy/awkward. The cursor landing its tip
        # on the element is the indicator. (Set a tuple here to bring it back.)
        self._ring = None
