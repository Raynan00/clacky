...
LOCATE_RE = re.compile(
    ...
    r"locate\s+the|highlight\s+the|"
    # natural references to a specific on-screen thing → point at it too
    r"what('?s|\s+is|\s+does|\s+are)\s+(this|that|it|these|those)|"
    r"(tell\s+me\s+about|explain)\s+(this|that|it))\b",
    re.IGNORECASE,
)
