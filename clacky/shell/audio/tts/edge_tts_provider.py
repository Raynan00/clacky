...
class EdgeTTSProvider(BaseTTS):
    ...
    async def synth(self, text: str) -> bytes:
        """Synthesize to MP3 bytes WITHOUT playing. Lets a caller time playback
        against other events — e.g. fire the pointer exactly when audio starts,
        so cursor and voice land together instead of the cursor racing ahead."""
        if not text.strip():
            return b""
