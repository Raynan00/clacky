"""
Clacky for Windows — Entry Point.
...
def _copilot_login_flow(tray, panel, manager):
    ...
    def _on_code(user_code: str, verification_uri: str):
        ...
        msg = (
            ...
            f"3. Click Authorize — Clacky will sign in automatically."
        )
...
def main():
    ...
    app.setApplicationName("Clacky")
    app.setApplicationDisplayName("Clacky - AI Companion")
    ...
    def _on_doc_dropped(path: str):
        ...
        tray.show_notification(
            ...
            f"{path}\nAsk Clacky about it now." if ok else
    ...
    def _wf_stop():
        ...
        if summary:
            tray.show_notification(
                "Workflow Captured",
                "Sent to Clacky as context. Ask: 'what did I just do?'"
            )
    ...
    def _attach_doc():
        ...
        path, _ = QFileDialog.getOpenFileName(
            None, "Attach a document for Clacky",
    ...
    def _switch(name: str):
        ...
        tray.show_notification("Clacky", f"Switched to {name}")
    ...
    def _save_diagnostics():
        ...
        base = os.environ.get("LOCALAPPDATA") or os.path.expanduser("~")
        out = Path(base) / "Clacky" / f"diagnostics-{datetime.datetime.now():%Y%m%d-%H%M%S}.txt"
        try:
            providers_d = cfg.describe()
        except Exception:
            providers_d = {}
        report = []
        report.append(f"Clacky diagnostics — {datetime.datetime.now().isoformat()}")
    ...
    tray.show_notification(
        "Clacky is running",
        f"Say 'Clacky' or hold {cfg.hotkey}  |  LLM: {providers['llm']}",
    )
    ...
    try:
        ...
        if os.environ.get("CLACKY_FORCE_SETUP", "").strip() in ("1", "true", "yes"):
            wiz = SetupWizard()
            wiz.show()
            _setup_keepalive[0] = wiz
        else:
