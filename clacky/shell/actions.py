    ...
    async def _bg_worker(self, tid: int, description: str):
        ...
        try:
            ...
            if harness.harness_available():
                ...
                # open it. No host list: the harness only surfaces links that
                # the delivering server actually returned in a tool result, so
                # "open where she did the job" is backed by evidence, not by
                # the model's say-so.
                if res.links:
                    import webbrowser
                    webbrowser.open(res.links[0])
